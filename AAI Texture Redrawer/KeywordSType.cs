...
{
    class KeywordSType : TextureType
    {
        private int length;
        private float K = 0.78f;
        private int K2 = 7;
        public KeywordSType() : base("Keyword", "Open Sans SemiBold", 27f, Color.FromArgb(255, 255, 255, 255), //Ruptured balloon
            Resources.Keyword_sTempl,
            new StringFormat
            {
        ...
        public override Image DrawText(string[] text)
        {
            if (text[1] != "")
            {
                Image res = SetTextOn(text[0], -100, 0, Template.Width + 200, Template.Height / 2, Template);
                res = SetTextOn(text[1], -100, Template.Height / 2 - 25, Template.Width + 200, Template.Height / 2, res);
                return res;
            }
            else
            {
                Image res = SetTextOn(text[0], -100, 24, Template.Width + 200, Template.Height / 2, Template);
                return res;
            }
        }
        ...
        {
            Color MainColor = Color.FromArgb(255, 0, 34, 85);
        ...
        public Image SetTextOn(string text, int x, int y, int width, int height, Image orig)
        {
            ...
            using (Graphics graphics = Graphics.FromImage(tmp))
            {
                ...
                {
                    DrawTextWithInterval(graphics, text, font, new SolidBrush(TextColor), new Rectangle(0, 0, width * 3, height * 3), Format, -45);
                }
            }
            ...
            using (Graphics graphics = Graphics.FromImage(res))
            {
                using (Font font = new Font(FontName, FontSize))
                {
                    ...
                    graphics.DrawImage(tmp, new Rectangle(rx - 1, y - 1, width, height));
                }
            }
